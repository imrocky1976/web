...
class APITestCase(unittest.TestCase):
    ...
    def test_token_auth(self):
        # add a user
        u = User(email='john1@example.com', password='cat', username='john1', confirmed=True)
        db.session.add(u)
        db.session.commit()
    ...
    def test_unconfirmed_account(self):
        ...
        u = User(email='john2@example.com', password='cat', confirmed=False,
                 username='john2')
        db.session.add(u)
        db.session.commit()
    ...
    def test_posts(self):
        # add a user
        u = User(email='john3@example.com', password='cat', confirmed=True,
                 username='john3')
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.post(
            '/api/v1.0/posts/',
            headers=self.get_api_headers('john3@example.com', 'cat'),
    ...
    def test_users(self):
        # add two users
        u1 = User(email='john4@example.com', username='john4',
        ...
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['username'], 'john4')
        response = self.client.get(
    ...
    def test_comments(self):
        ...
        u1 = User(email='john5@example.com', username='john5',
                  password='cat', confirmed=True, role=r)
        u2 = User(email='susan1@example.com', username='susan1',
        ...
        response = self.client.post(
            ...
            headers=self.get_api_headers('susan1@example.com', 'dog'),
        ...
        response = self.client.get(
            url,
            headers=self.get_api_headers('john5@example.com', 'cat'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual('http://localhost' + json_response['url'], url)
        self.assertEqual(json_response['body'],
